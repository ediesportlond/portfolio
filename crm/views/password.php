<?php
    //check OT is valid.if not redirect to login page.
    if($_GET['forgot']){
        $obj['otp'] = $_GET['otp'];
        checkOTP($conn, $obj);
    } else if($_SESSION['id'] == ''){
...
<main class='container-md text-center pt-4'>
    <div class='border rounded bg-light pb-2'>
    <?php
        //field variance when logged in vs forgot password.
        if(!$_GET['forgot']){
            echo "<form action='?page=password' method='post'>
                    <label class='form-label mt-4' required>Current password</label>
                    <input class='form-control form-control-lg form-width' type='password' name='currentPass'>";
        } else {
            echo "<form action='?page=password&forgot=".$_GET['forgot']."' method='post'>
                    <input type='password' style='display:none' name='otp' value ='".$_GET['forgot']."'>";
        }
    ?>
    ...
    <input class='btn btn-primary mt-4' name='changePass' value='Change Password' type='submit'>
    </form>
    </div>
</main>
...
<script>
    ...
    $('[name=changePass]').on('click',function(e){
